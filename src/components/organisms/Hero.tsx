...
const Hero = () => {
  ...
  const [textContent, setTextContent] = useState<'carp' | 'carpool'>('carp');
  const [pageReady, setPageReady] = useState(false);
  
  useEffect(() => {
    // Handle typing animation
    const timer1 = setTimeout(() => {
      setTextContent('carpool');
      
      // Blink cursor for 1 more second then stop
        ...
        <div className="flex flex-col lg:flex-row items-center space-y-12 lg:space-y-0 lg:space-x-8">
          ...
          <div className="w-full lg:w-1/2 space-y-8 text-center lg:text-left">
            <div className="space-y-3">
              <div 
                className={`inline-block bg-carp-lightBlue text-carp-blue rounded-full px-4 py-1 font-medium dark:bg-blue-900/50 transition-opacity duration-500 ${pageReady ? 'animate-fade-in' : 'opacity-0'}`}
              >
                Where the journey matters more than the destination.
              </div>
              <h1 className="font-bold text-foreground">
                <span className="text-carp-blue">
                  {textContent.substring(0, 4)}
                </span>
                <span className="text-foreground">
                  {textContent === 'carpool' ? 'ool' : ''}
                </span>
                <span className={`inline-block w-[4px] h-[1em] bg-carp-blue align-middle ${showTypingCursor ? 'animate-pulse-light' : 'opacity-0'}`}></span>
                {pageReady && (
                  <><br />
            ...
            {pageReady && (
              <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start space-y-4 sm:space-y-0 sm:space-x-4 animate-fade-in" style={{ animationDelay: '400ms' }}>
                <Button 
                  size="lg" 
          ...
          {/* Right column / Visual content - conditionally render based on pageReady */}
          {pageReady && (
            <div className="w-full lg:w-1/2 relative animate-fade-in" style={{ animationDelay: '500ms' }}>
              <div className="relative flex justify-center lg:justify-end">
                {/* Phone mockup */}
                <div className="relative w-[280px] sm:w-[320px] h-[580px] sm:h-[640px] rounded-[40px] border-[10px] border-black dark:border-gray-800 bg-white/90 dark:bg-gray-900 overflow-hidden shadow-2xl">
                  {/* App screen content */}
                ...
                {/* Floating UI elements - only show when pageReady is true and adjust for mobile */}
                {pageReady && (
                  <FloatingUIContainer 
                    position="center" 
                    xOffset="-80%" 
                    yOffset="-10vh"
                    className="hidden lg:flex" // Hide on mobile, show on larger screens
                  >
                    <FloatingUI
                ...
                {/* Mobile floating UI - simplified and stacked vertically */}
                {pageReady && (
                  <div className="absolute -right-4 top-0 flex flex-col gap-3 lg:hidden">
                    <FloatingUI
                      title="Save $240/month"
                      text="with carpooling"
                      icon="💸"
                      bgColor="bg-yellow-100 dark:bg-yellow-900/30"
                      textColor="text-yellow-600 dark:text-yellow-400"
                      animationDelay="400ms"
